...
gulp.task('pages',  function () {
  function replaceTemplates(file) {
    var fileContent = String(fs.readFileSync("./modules/pagination-controls-cmp.html", "utf8"));
    file.contents = new Buffer(String(file.contents).replace("{COMPONENT_TEMPLATE}", fileContent.replace(/(\r\n|\n|\r)/gm,"").replace(/"/g, '\\"')));
  }

  return gulp.src('./build/pagination-controls-cmp.js')
